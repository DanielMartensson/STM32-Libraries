...
static GPIO_TypeDef *GPIO_CS_PORT;
static uint16_t GPIO_CS_PIN;
...
void SD_init_handler(SPI_HandleTypeDef *hspi, GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin) {
	hspi_ = hspi;
	GPIO_CS_PORT = GPIOx;
	GPIO_CS_PIN = GPIO_Pin;
}
...
static void SPI_TxByte(BYTE data) {
	while (HAL_SPI_GetState(hspi_) != HAL_SPI_STATE_READY)
		;
	HAL_SPI_Transmit(hspi_, &data, 1, SPI_TIMEOUT);
}
...
static uint8_t SPI_RxByte(void) {
	...
	while ((HAL_SPI_GetState(hspi_) != HAL_SPI_STATE_READY))
		;
	HAL_SPI_TransmitReceive(hspi_, &dummy, &data, 1, SPI_TIMEOUT);
